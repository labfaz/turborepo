...
const bodyFormParse = (values: any) => {
  ...
  if (values.artist?.address?.number)
    formData.append(
      ...
      toNumString(values.artist?.address?.number)
    );
  formData.append(
    'artist[address][complement]',
    values.artist?.address?.complement
  );
  formData.append(
    'artist[address][residency]',
    values.artist?.address?.residency
  );
  formData.append('artist[address][state]', values.artist?.address?.state);

  formData.append(
    'artist[technical][is_drt]',
    values.artist?.technical?.is_drt
  );
  formData.append('artist[technical][drt]', values.artist?.technical?.drt);

  formData.append(
    'artist[technical][is_ceac]',
    values.artist?.technical?.is_ceac
  );
  formData.append('artist[technical][ceac]', values.artist?.technical?.ceac);

  formData.append(
    'artist[technical][formation]',
    values.artist?.technical?.formation
  );
  formData.append(
    'artist[technical][profession]',
    values.artist?.technical?.profession
  );

  formData.append(
    'artist[technical][is_cnpj]',
    `${values.artist?.technical?.is_cnpj}`
  );
  formData.append('artist[technical][cnpj]', values.artist?.technical?.cnpj);
  formData.append(
    'artist[technical][cnpj_type]',
    values.artist?.technical?.cnpj_type
  );
  // formData.append('artist[technical][name_enterprise]', values.artist?.technical?.name_enterprise)

  formData.append(
    'artist[technical][areas][0][name]',
    values.artist?.technical?.areas.name
  );
  formData.append(
    'artist[technical][areas][0][technical_formation]',
    values.artist?.technical?.areas.technical_formation
  );
  formData.append(
    'artist[technical][areas][0][describe]',
    values.artist?.technical?.areas.describe
  );
  formData.append(
    'artist[technical][areas][0][started_year]',
    values.artist?.technical?.areas.started_year
  );

  if (values.artist?.technical?.areas?.certificate) {
    values.artist.technical.areas.certificate.forEach(
      (certificate: string, index: number) => {
        formData.append(
          `artist[technical][areas][0][certificate][${index}][name]`,
          certificate
        );
      }
    );
  ...
  formData.append(
    ...
    values.artist.contact?.instagram
  );
...
export const SignUp = async (values: any) => {
  ...
  return api
    .post<SuccessObject>('/user/create', formParsed, {
      headers,
    })
    .then((res) => {
      return res;
