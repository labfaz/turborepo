...
export const useUsersList = () =>
  useQuery<UsersList, ErrorObject>(`/users/list`, fetchUsersList, {
    enabled: false,
  });
...
const isLGBTQ = (user: User) =>
  !(
    user.artist.gender_specifics === GenderSpecific.CIS ||
    user.artist.gender_specifics === GenderSpecific.NONE
  ) ||
  !(
    user.artist.sexual_orientation === 'heterosexual' ||
    isPrefiroNao(user.artist.sexual_orientation)
  );
