...
const mergeFlags = (...regexList: RegExp[]) =>
  regexList
    .map((r) => r.flags)
    .filter((f, i, a) => a.indexOf(f) === i)
    .join('');

export const or = (...regexList: RegExp[]) =>
  new RegExp(
    regexList.map((r) => `${r.source}`).join('|'),
    mergeFlags(...regexList)
  );

export const capture = (groupName: string, regex: RegExp) =>
  new RegExp(`(?<${groupName}>${regex.source})`, regex.flags);

export const nonCapture = (regex: RegExp) =>
  new RegExp(`(?:${regex.source})`, regex.flags);

// TODO: do the correct user extraction and transformation into link before sending this to the backend

// regexList for username or link to user page
