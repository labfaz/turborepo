...
export const iconDict: { [i in IconName]: StaticImageData } = {
  ceac: ceacImg,
  cnpj: cnpjImg,
  drt: drtImg,
...
const isIconName = (label: string): label is IconName =>
  IconNames.includes(label as IconName);

// get icon
export const getIcon = (label: string) =>
  isIconName(label) ? iconDict[label] : undefined;
